...
public class Connect
{
	...
	public static void main(String args[]) throws ClassNotFoundException,SQLException
	{
		...
		try
		{
	        ...
	        String createTeam = "CREATE TABLE Team (teamID INTEGER PRIMARY KEY, supervisor INTEGER, name VARCHAR(50) NOT NULL, affiliation VARCHAR(200) NOT NULL, supervisor INTEGER NOT NULL, key INTEGER UNIQUE, FOREIGN KEY supervisor REFERENCES SuperUser (userID)) ENGINE=INNODB;";
			...
			String createCard = "CREATE TABLE Card (cardID INTEGER PRIMARY KEY, listID INTEGER, name VARCHAR(50) NOT NULL, priority INTEGER CHECK (priority <= 6 AND priority >= 0), description VARCHAR(1000) NOT NULL, dueDate DATETIME NOT NULL, archived VARCHAR(5) CHECK (archived IN ('True','False')), 12 Design Report UptownFANK finished VARCHAR(5) CHECK (finished IN ('True','False')), FOREIGN KEY listID REFERENCES List (listID)) ENGINE=INNODB;";
	        
			String createPerformsTask = "CREATE TABLE PerformsTask (cardID INTEGER, userID INTEGER, PRIMARY KEY (cardID, userID), FOREIGN KEY userID REFERENCES User (userID), FOREIGN KEY cardID REFERENCES Card (cardD)) ENGINE=INNODB;";
	        
			String createCheckList = "CREATE TABLE CheckList (checklistID INTEGER PRIMATY KEY, relatedCard INTEGER, name VARCHAR(50) NOT NULL, checkStatus VARCHAR(5) CHECK (checkStatus IN ('True','False')), description VARCHAR(1000) NOT NULL, relatedCard INTEGER, FOREIGN KEY relatedCard REFERENCES Card (cardID)) ENGINE=INNODB;";
